...
describe('ZkSafebox-ext-test', function () {
    let accounts
    let provider
    let zkSafebox
    let mockDeposit
    ...
    it('deploy', async function () {
        ...
        mockDeposit = await MockDeposit.deploy(zkSafebox.address)
        await mockDeposit.deployed()
        console.log('mockDeposit deployed:', mockDeposit.address)
    })
    ...
    it('withdrawToERC20Receiver', async function () {
        let psw = 'abc123'
        ...
        if (res === true) {
            ...
            await zkSafebox.withdrawToERC20Receiver(proof, pswHash, usdt.address, m(30, 18), allHash, mockDeposit.address)
            console.log('withdrawToERC20Receiver done')
    ...
    it('transfer', async function () {
        ...
        let tokenAddr = usdt.address //hex or int
        let amount = s(m(40, 18)) //hex or int
        let input = [stringToHex(psw), tokenAddr, amount]
        let data = await snarkjs.groth16.fullProve({in:input}, "./zk/main3_js/main3.wasm", "./zk/circuit_final.zkey")
        ...
        if (res === true) {
            ...
            await zkSafebox.transfer(proof, pswHash, usdt.address, m(40, 18), allHash, accounts[1].address)
    ...
    async function print() {
        ...
        console.log('mockDeposit', 'usdt:', d(await usdt.balanceOf(mockDeposit.address), 18))
        console.log('mockDeposit token2user2amount', d(await mockDeposit.token2user2amount(usdt.address, accounts[0].address), 18))
        console.log('')
    }
